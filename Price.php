...
class Price
{
    ...
    protected function _applyOptionsPrice($product, $qty, $finalPrice)
    {
        ...
        if ($optionIds) {
            $optionsIdsArray = explode(',', $optionIds->getValue());
            $hochbeeteOptionsIds = [22,25,28,31,34,37,4,7,10,13,16,19];
            $intersect = array_intersect ($optionsIdsArray, $hochbeeteOptionsIds);
            if (count($intersect) > 0) {
                return $this->_calculateFormulaPrice($product, $qty, $finalPrice,$intersect);
            }
            else {
    ...
    protected function _calculateFormulaPrice($product, $qty, $finalPrice, $optionsIds ) {

        $name = $product->getName();
        ///////////////// Constants //////////////////////

        // basic=>203;classic=>0.484 for classic
        // basic=>77;classic=>0.34; for basic

        $basic = $name == "HOCHBEET CLASSIC" ? 203 : 77;
        $classic = $name == "HOCHBEET CLASSIC" ? 0.484 : 0.34;

        /////////////////////////////////////////////////
        ////// Variables initialization ///

        // Länge (in cm) => a; Breite (in cm) => b;

        $a = 50;
        $b = 50;

        // Nagetiergitter + Trockenschutzfolie=>z1;

        $z1 = 0;

        // Handlauf gewölbt, mit Schraubenset=>z2;

        $z2 = 0;

        // Schneckenkante mit Schrauben=>z3;

        $z3 = 0;

        //Aussteifungsset (ab 240 cm Länge empfohlen)=>z4;

        $z4 = 0;

        /////////////////////////////////////////////////
        ////// Variable assigments by product options ///
        ...
        foreach($optionsIds as $optionId) {
            if ($option = $product->getOptionById($optionId)) {
                ...
                switch ($optionId) {
                    ...
                    case 22:
                        $a = $optionValue;
                        if ($a < 50) throw new FrameworkException();
                        break;
                    case 7:
                    case 25:
                        $b = $optionValue;
                        if ($b < 50) throw new FrameworkException();
                        break;
                    case 10:
                        $z1 = $optionValue == 1 ? 1 : 0;
                        break;
                    case 28:
                        $z1 = $optionValue == 25 ? 1 : 0;
                        break;
                    case 13:
                        $z2 = $optionValue == 7 ? 1 : 0;
                        break;
                    case 31:
                        $z2 = $optionValue == 31 ? 1 : 0;
                        break;
                    case 16:
                        $z3 = $optionValue == 13 ? 1 : 0;
                        break;
                    case 34:
                        $z3 = $optionValue == 37 ? 1 : 0;
                        break;
                    case 19:
                        $z4 = $optionValue == 19 ? 1 : 0;
                        break;
                    case 37:
                        $z4 = $optionValue == 43 ? 1 : 0;
                        break;
                    default:
                        break;
                }
        ...
        ///////////////////////////////////////////////////
        ///////////////////////////////////////////////////
        // Static variable (formula based) ////////////////
        
        /////////////////////////////////////////////////
        // nagetiergitter=>({a}+{b})*2*5/100*{z1};

        $nagetiergitter = (( $a + $b )* 2 * 5 / 100) * $z1;

        /////////////////////////////////////////////////
        // handlauf=>({a}+{b})*2*12.5/100*{z2};

        $handlauf = (($a+$b)*2*12.5/100)*$z2;

        /////////////////////////////////////////////////
        //schneckenkante=>(({a}+{b})*2+200)/200*14*{z3};
        ...
        $aussteifungsset = $name == "HOCHBEET CLASSIC" ? (79.9 * $z4) : (39.9 * $z4);
        //aussteifungsset=>39.9*{z4};

        $price = ($a + $b) *2 * $classic + $basic + $nagetiergitter + $handlauf + $aussteifungsset + $schneckenkante;
